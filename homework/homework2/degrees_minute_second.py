...
def dms_to_radian(degrees: int, minutes: int, seconds: int) -> tuple [float, float, float, float]:
    if not all(1 <= t <= 59 for t in (minutes, seconds)) or not 1 <= degrees <= 180:
        raise ValueError('out of range')
    decimal_degrees = degrees + minutes / 60 + seconds / 3600
    rad = decimal_degrees * (math.pi / 180)
    return round(rad, 2), round(math.sin(rad), 2), round(math.cos(rad), 2), round(math.tan(rad), 2)
