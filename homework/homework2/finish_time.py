...
def find_time(
    first_time: tuple[int, int, int], second_time: tuple[int, int, int]
) -> int:
    if (
        not all(0 <= i <= 59 for i in (first_time[2], second_time[2]))
        or not 1 <= first_time[0] <= 2
        or not 0 <= first_time[1] <= 30
        or not 31 <= second_time[1] <= 59
    ):
