...
@pytest.mark.parametrize('h, w, h1, w1, expected_result', [(16, 22, 78, 46, True), (29, 80, 8, 22, False), (55, 92, 36, 62, False)])
def test_cabinet_size(h, w, h1, w1, expected_result):
    assert cabinet_size(h, w, h1, w1) == expected_result


@pytest.mark.parametrize('h, w, h1, w1', [(3, 8, 22, 115), (22, 200, -50, 20), (222, 118, -32, 562)])
def test_cabinet_size_out_of_range(h, w, h1, w1):
    with pytest.raises(ValueError, match='out of range'):
        cabinet_size(h, w, h1, w1)


@pytest.mark.parametrize('month, expected_result', [(10, "Autumn"), (5, 'Spring'), (8, "Summer")])
...
@pytest.mark.parametrize('temperature_b, temperature, weather, expected_result', [(36.1, 22, 'sunny', 'go walk'), (36.4, 20, 'cloudy', 'The weather is bad'), (36.3, 30, 'worm', 'Extremal temperature')])
...
@pytest.mark.parametrize('temperature_b, temperature, weather', [(-32, 18, 'sunny'), (36.6, -52, 'cloudy'), (32.3, 58, 'snow')])
def test_time_to_walk_out_of_range(temperature_b, temperature, weather):
    with pytest.raises(ValueError, match='out of range'):
        time_to_walk(temperature_b, temperature, weather)


@pytest.mark.parametrize('slices, k_slices, expected_result', [(20, 10, True), (19, 6, False), (20, 8, False)])
...
@pytest.mark.parametrize('x, y, z, expected_result', [(15.83, 2.26, 0.45, 0.92), (2, 2, 4, "You can't divide by zero"), (11.76, -16.32, -2.33, 2.04)])
...
@pytest.mark.parametrize('money, drinker, gave, misser, expected_result', [(True, True, False, False, "he is a drinker"), (False, False, False, True, "you have no money"), (True, False, True, False, "you can land")])
...
@pytest.mark.parametrize('money, drinker, gave, misser', [(1, 2, 3, 4), ("you can land", "you are misser", "he doesn't pay his debts", "he is a drinker"), (True, 12, False, "he is a drinker")])
...
@pytest.mark.parametrize('h_start1, m_start1, s_start1,'
...
def test_intersecting_events(h_start1,m_start1, s_start1,
                             h_finish1, m_finish1, s_finish1,
                             h_start2, m_start2, s_start2,
                             h_finish2, m_finish2, s_finish2, expected_result):
    assert intersecting_events(h_start1, m_start1, s_start1,
                               h_finish1, m_finish1, s_finish1,
                               h_start2, m_start2, s_start2,
                               h_finish2, m_finish2, s_finish2) == expected_result
...
@pytest.mark.parametrize('h_start1, m_start1, s_start1,'
                         'h_finish1, m_finish1, s_finish1,'
                         'h_start2, m_start2, s_start2,'
                         'h_finish2, m_finish2, s_finish2', [(8, 18, 234, 43, 12, 45, 78, 34, 78, -12, 54, 98),
...
def test_intersecting_events(h_start1,m_start1, s_start1,
    ...
    with pytest.raises(ValueError, match="out of range"):
        intersecting_events(h_start1,m_start1, s_start1,
                             h_finish1, m_finish1, s_finish1,
                             h_start2, m_start2, s_start2,
                             h_finish2, m_finish2, s_finish2)


@pytest.mark.parametrize('x1, y1, w1, h1,'
                         'x2, y2, w2, h2, expected_result',
...
def test_find_intersection(x1, y1, w1, h1,
                           x2, y2, w2, h2, expected_result):
    assert find_intersection(x1, y1, w1, h1,
                             x2, y2, w2, h2) == expected_result
...
def test_find_intersection_out_of_range(x1, y1, w1, h1,
                                        x2, y2, w2, h2):
    with pytest.raises(ValueError, match="out of range"):
        find_intersection(x1, y1, w1, h1, x2, y2, w2, h2)


@pytest.mark.parametrize('ruble, expected_result', [(53.02, 'рубля'), (89.54, 'рублей'), (81.29, 'рубль')])
...
@pytest.mark.parametrize('ruble, expected_result', [(53.02, 'копейки'), (81.29, 'копеек'), (97.91, 'копейка')])
...
@pytest.mark.parametrize('current, command1, command2, expected_result', [('east', -1, 2, 'north'), ('south', 2, 2, 'south'), ('east', 1, -1, 'east')])
...
@pytest.mark.parametrize('current, command1, command2', [('east', 0, 0), ("east", -5, 2), ('east', 12, 54)])
def test_radar_direction_out_of_range(current, command1, command2):
    with pytest.raises(ValueError, match='out of range'):
        radar_direction(current, command1, command2)


@pytest.mark.parametrize('day_k, day_n, expected_result', [(124, 6, "Wednesday"), (47, 4, "Monday"), (289, 6, "Sunday")])
