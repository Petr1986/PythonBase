...
@pytest.mark.parametrize('num1, num2, num3, expected_result', [(90, 21, 96, True), (77, 25, 35, False), (42, 34, 22, True)])
def test_three_integers(num1, num2, num3, expected_result):
    assert three_integers(num1, num2, num3) == expected_result


@pytest.mark.parametrize('num1, num2, num3', [(-12, 50, 64), (10, 112, 58), (42, 89, -545)])
def test_three_integers_out_of_range(num1, num2, num3):
    with pytest.raises(ValueError, match='out of range'):
        three_integers(num1, num2, num3)


@pytest.mark.parametrize('num1, num2, num3, expected_result', [(-1.17, 1.68, 2.91, True), (7.93, 0.31, 2.04, False), (3.29, 3.39, 0.76, False)])
def test_three_real_numbers(num1, num2, num3, expected_result):
    assert three_real_numbers(num1, num2, num3) == expected_result


@pytest.mark.parametrize('num1, num2, num3', [(-7.3, 2.8, 9.4), (3.2, 11.5, -4.8), (-1.3, 6.1, -12.6)])
def test_three_real_numbers_out_of_range(num1, num2, num3):
    with pytest.raises(ValueError, match='out of range'):
        three_real_numbers(num1, num2, num3)


@pytest.mark.parametrize('name, expected_result', [('Kirill', 'Добро пожаловать, Kirill'), ('Petr', 'Добро пожаловать, Petr'), ('', 'Пожалуйста авторизуйтесь')])
...
@pytest.mark.parametrize('x, y, expected_result', [(6, 2, 'black'), (1, 2, 'white'), (1, 1, 'black')])
...
@pytest.mark.parametrize('x, y, expected_result', [(16, -13, 'fourth quarter'), (-6, -11, 'third quarter'), (-3, 13 , 'second quarter')])
...
@pytest.mark.parametrize('num, expected_result', [(1661, 'Да'), (1656, 'Нет'), (6814, 'Нет')])
