...
from homework.homework1.perimeter_and_area_of_a_triangle import (
    area_of_triangle,
    distance,
    calculation_perimetr,
)
...
def test_conversation_out_of_range(x):
    with pytest.raises(
        ValueError, match="Temperature should be in the range from -273 to 1000"
    ):
        conversion(x)
...
def test_equation1_out_of_range(a, x):
    with pytest.raises(
        ValueError,
        match="The value of a must be in the range from -20 to 20|The value of x must be in the range from 0 to 100",
    ):
        equation1(a, x)
...
def test_equation2_out_of_range(a, b, c, d, x):
    with pytest.raises(
        ValueError,
        match="The value must be in the range from -20 to 20|The value must be in the range from 0 to 100",
    ):
        equation2(a, b, c, d, x)
...
@pytest.mark.parametrize("students", [-15, 433, 115])
def test_quantity_desks_out_of_range(students):
    with pytest.raises(
        ValueError, match="The number of students must be between 1 and 48"
    ):
        quantity_desks(students)


@pytest.mark.parametrize(
    "students, except_result", [([3, 32, 31], 34), ([14, 17, 6], 19), ([25, 8, 24], 29)]
)
...
@pytest.mark.parametrize(
    "p1, p2, p3, except_result",
    [
        ((-18, 9), (-12, -10), (-14, 2), 17.00),
        ((1, 2), (11, 15), (-19, -3), 105.00),
        ((4, 7), (16, 8), (-7, 11), 58.50),
    ],
)
...
@pytest.mark.parametrize(
    "p1, p2, p3",
    [((25, -35), (124, -23), (115, -122)), ((-164, 152), (-34, 194), (-141, 134))],
)
def test_area_of_triangle_out_of_range(p1, p2, p3):
    with pytest.raises(
        ValueError,
        match="The x coordinates must be in the range from -20 to 20|The y coordinates must be in the range from -20 to 20",
    ):
...
@pytest.mark.parametrize(
    "a, b, except_result", [(54, 39, (1, 15)), (75, 17, (4, 7)), (90, 30, (3, 0))]
)
...
def test_calculation_segments_out_of_range(a, b):
    with pytest.raises(
        ValueError,
        match="The value of a must be in the range from 50 to 100|The value of b must be in the range from 1 to 49",
    ):
        calculation_segments(a, b)


@pytest.mark.parametrize(
    "first_number, second_number, except_result",
...
def test_concatenation_out_of_range(first_number, second_number):
    with pytest.raises(
        ValueError,
        match="The number must be greater than 99|The number must be less than 1000",
    ):
...
def test_result_out_of_range(a, b, c):
    with pytest.raises(
        ValueError,
        match="Sides A and B must be in the range from 50 to 100|Side C must be in the range from 1 to 49",
    ):
        result(a, b, c)


@pytest.mark.parametrize(
    "k, except_result", [(2144, (21, 44)), (1825, (18, 25)), (3983, (39, 83))]
)
...
def test_cost_out_of_range(k):
    with pytest.raises(
        ValueError, match="The price must be in the range from 1 to 10000"
    ):
        cost(k)


@pytest.mark.parametrize(
    "a, b, c, d, except_result",
    [
        (-99, 68, -14, 43, -20195.99),
        (-41, 45, -88, -11, -5535.00),
        (83, 17, 56, 36, 4233.00),
    ],
)
...
@pytest.mark.parametrize(
    "a, b, c, d", [(-125, 100, 182, 94), (-11, 666, 151, -237), (965, -554, 98, 2)]
)
def test_calculation_expression_out_of_range(a, b, c, d):
    with pytest.raises(
        ValueError,
        match="Numbers must be between -100 and 100|Numbers must not be equal to 0",
    ):
        calculation_expression(a, b, c, d)


@pytest.mark.parametrize("x, except_result", [(906, 15), (870, 15), (655, 16)])
...
def test_calculation_sum_num_out_of_range(x):
    with pytest.raises(ValueError, match="The number must be between 100 and 999"):
        calculation_sum_num(x)


@pytest.mark.parametrize("x, except_result", [(8804, 8), (9531, 5), (2132, 1)])
...
def test_define_out_of_range(x):
    with pytest.raises(ValueError, match="The number must be between 100 and 10000"):
        define(x)


@pytest.mark.parametrize("y, except_result", [(3263, 2633), (6642, 6426), (4398, 3984)])
...
def test_find_number_out_of_range(y):
    with pytest.raises(ValueError, match="The number must be between 10 and 10000"):
        find_number(y)
